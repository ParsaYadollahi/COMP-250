...
public class Chaining {
    ...
    public int chain (int key) {
        ...
        int modulus = (int) Math.pow(2, w);

        int hash_value = ((A)*key)%modulus >> (w-r);
        return hash_value;
    }
    ...
    public int insertKey(int key) {
        //TODO: implement this and change the return statement
        int hash_value = chain(key);
        List<Integer> sub_list = Table.get(hash_value);

        sub_list.add(key); // add value into arraylist
        return (sub_list.size()-1);
    }
